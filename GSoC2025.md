...
## 21.8.2025 - Store user_tally data in the HDF5 output file

With commit [d778bdf](https://github.com/ir2-lab/OpenTRIM/commit/d778bdf1dbf6162928df9d9ac3b15227516117eb) there is an initial
implementation for saving user_tally to HDF5.

The path inside the HDF5 file for storing all user_tally info is `/user_tally/(tally id)/...`, where `(tally id)` is the user specified id.

The initial implementation saves only the multidimentional bin data (variable `user_tally::data_`) at `/user_tally/(tally id)/data`.
A 2nd variable called `data_sem` with same dimensions as `data` are the corresponding errors for each bin (sem = "standard error of the mean").

> TODO: add code in `h5serialize.cpp` to store all user_tally metadata as follows:

1. `zaxis, xzvec, org`
   These are 3D vectors that define the coordinate system of the tally.
   - Define a user_tally member function to get the respective variable, e.g. 
     ```cpp
     const vector3& zaxis() const { return par_.zaxis; }`
     ```
     See the function `user_tally::id()`
   - Save to HDF5 using the function `dump_vector()`

2. type of coordinates
   Save as a string ("xyz", "cyl" or "sph"). 
   Write a user_tally member function to return the string and save it to HDF5 using `dump()`

3. event
   Save as a string (e.g. "IonStop").
   Write a user_tally member function to return a string depending on the event type. See for example `tally::arrayName()`
   Save it to HDF5 using `dump()`

4. "bin_names"
   This should be a vector of strings (`std::vector<std::string>`) that contains the names of each binning variable. E.g. `{"x", "y", "z"}` or `{"rho","z","atom_id"} `.
   This must be first created and then stored to HDF5 with `dump()`. See lines 450-465 of `h5serialize.cpp` for an example.
   A function is needed in `user_tally` that returns a string depending on the `variable_code`, e.g., `const char* variable_code_str(variable_code c)`. It will return `"x"` for `cX`, etc. Follow the example of `tally::arrayName()`

5. The actual bins
   Each vector of bin edges should be written as:
   ```
   /user_tally/(tally id)/bins/0
   /user_tally/(tally id)/bins/1
   ...
   ``` 

The final structure in the HDF5 file will look like:
```
/
| - user_tally/
    | - (tally id)/
