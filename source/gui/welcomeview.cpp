...
WelcomeView::WelcomeView(MainUI *iui, QWidget *parent) : QWidget{ parent }, ionsui(iui)
{
    ...
    btOpen->setMinimumSize(QSize(w, h));
    {
        ...
        connect(action, &QAction::triggered, this, &WelcomeView::onOpenJson);
        action = toolMenu->addAction("Open HDF5 file");
        connect(action, &QAction::triggered, this, &WelcomeView::onOpenH5);
    ...
    btSave->setMinimumSize(QSize(w, h));
    {
        ...
        connect(actSaveJson, &QAction::triggered, this, &WelcomeView::onSaveJson);
        actSaveH5 = toolMenu->addAction("Save Config+Data to HDF5");
        actSaveH5->setEnabled(false);
        connect(actSaveH5, &QAction::triggered, this, &WelcomeView::onSaveH5);
    ...
    btSaveAs->setMinimumSize(QSize(w, h));
    {
        ...
        connect(actSaveJsonAs, &QAction::triggered, this, &WelcomeView::onSaveJsonAs);
        actSaveH5As = toolMenu->addAction("Save Config+Data to HDF5 As ...");
        connect(actSaveH5As, &QAction::triggered, this, &WelcomeView::onSaveH5As);
    ...
    connect(buttonGrp, &QButtonGroup::idClicked, this, &WelcomeView::changeCenterWidget);
    connect(exampleList, &QListWidget::currentItemChanged, this, &WelcomeView::exampleSelected);
    connect(exampleList, &QListWidget::itemDoubleClicked, this, &WelcomeView::onOpenExample);
    connect(recentFilesTree, &QTreeWidget::currentItemChanged, this,
            &WelcomeView::onRecentFileSelected);
    connect(recentFilesTree, &QTreeWidget::itemDoubleClicked, this, &WelcomeView::onOpenRecent);
...
{
    if (!userDiscardCurrentSim("Open Example"))
        return;
...
void WelcomeView::onDriverStatusChanged()
{
    ...
    auto s = D->status();
    actSaveJson->setEnabled((D->isModified() || D->isTemplate()));
    actSaveH5->setEnabled((D->isModified() || D->isTemplate()) && s == McDriverObj::mcIdle);
...
void WelcomeView::onOpenRecent()
{
    ...
    if (!userDiscardCurrentSim("Open recent file ..."))
        return;
...
{
    if (!userDiscardCurrentSim("New simulation"))
        return;
...
void WelcomeView::onFileNameChanged()
{
    ...
    for (int i = 0; i < fileNames.size(); i++) {
        if (name == fileNames.at(i) && location == fileLocations.at(i)
...
void WelcomeView::updateRecentFiles()
{
    ...
    for (int i = 0; i < fileNames.size(); i++) {
        QTreeWidgetItem *item = new QTreeWidgetItem(
                recentFilesTree,
                QStringList({ fileNames.at(i), fileTitles.at(i), fileLocations.at(i) }));
