 ...
 *
 * All tables are 4-dimensional, \f$ [N_{at} \times N_x \times N_y \times N_z]\f$, where
 * \f$N_{at}\f$ is the number of atoms in the simulation (including the projectile ion)
 * and \f$N_{x,y,z}\f$ the number of cells along the 3 axes.
 * The exception is the very first table (i=0), which is
 * \f$ [N_{tally} \times N_{at}]\f$ and stores the totals of all
 * quantities summed over all cells.
 *
 * Tally scores are stored in double real number format.
 *
 * The element \f$A_k[i,i_x,i_y,i_z]\f$ of the \f$k\f$-th table stores the score
 * of the given quantity generated by an atom of id\f$=i\f$ in the
 * cell \f$(i_x,i_y,i_z)\f$. E.g., a vacancy of atom with id=2 generated in the cell
 * (10,0,0) will score +1 in \f$A_1[2,10,0,0]\f$.
 *
 * Internally, scoring uses the flat cell id returned by \ref grid3D::cellid(),
 * \f$ j = (i_x N_y + i_y) N_z + i_z\f$, which is the row-major flattening of the
 ...
 * Thus, in this case the 0-th slice of the respective table is all zeros.
 ...
 * on a target containig Fe2O3 (Fe with id=1, O with id=2) and bulk Fe (Fe with id=3).
 *
 * The tables are divided in groups:
 * - Energy deposition (Electronic, nuclear, etc)
...
public:
    ...
    enum tally_t {
        ...
        eElectronic, /**< Electronic energy loss  */
        eNuclear, /**< Nuclear energy loss */
     ...
     *
     * Energy : 3 (Electronic, Nuclear, PKA, Lost) atoms x cells
     *
...
public:
    ...
    void resetELossCounter() { eLossCounter_ = 0.0; }
    double eLossCounter() { return eLossCounter_; }
