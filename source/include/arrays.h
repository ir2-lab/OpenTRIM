...
class ArrayND
{
    ...
    struct Private
    {
        ...
        explicit Private(size_t i) : dim(1), buffer(i, Scalar(0)) { dim[0] = i; }

        Private(size_t i, size_t j) : dim(2), buffer(i * j, Scalar(0))
        {
            dim[0] = i;
            dim[1] = j;
        }

        Private(size_t i, size_t j, size_t k) : dim(3), buffer(i * j * k, Scalar(0))
        {
            dim[0] = i;
            dim[1] = j;
            dim[2] = k;
        ...
        Private(const std::vector<size_t> &d) : dim(d), buffer(calc_size(d), Scalar(0)) { }
        ...
        size_t idx(int i, int j) { return (dim.size() == 2) ? i * dim[1] + j : 0; }
    ...
    ArrayND(const Private &p) : P_(new Private(p)) { }
...
public:
    ...
    explicit ArrayND(const std::vector<size_t> &dim) : P_(new Private(dim)) { }
    /// Constructs a 1D array of dimension i
    explicit ArrayND(int i) : P_(new Private(i)) { }
    /// Constructs a 2D array of dimensions i x j
    explicit ArrayND(int i, int j) : P_(new Private(i, j)) { }
    /// Constructs a 3D array of dimensions i x j x k
    explicit ArrayND(int i, int j, int k) : P_(new Private(i, j, k)) { }
