...
void bind_config(py::module_ &m)
{
    ...
    atom_cls.def(py::init<>())
            ...
            .def("__repr__", [](const atom::parameters &a) {
                return "Atom(element=" + a.element.symbol + ", X=" + std::to_string(a.X)
    ...
    mat_cls.def(py::init<>())
            ...
            .def_readwrite("density", &material::material_desc_t::density, "Mass density [g/cm3].")
    ...
    py::class_<target::region> region_cls(
            m, "Region",
    ...
    py::class_<user_tally::bin_var_t> bins_cls(
            m, "UserTallyBins",
    ...
    bins_cls.def(py::init<>())
            ...
            .def_readwrite("Tdam", &user_tally::bin_var_t::Tdam,
            ...
            .def_readwrite("recoil_id", &user_tally::bin_var_t::recoil_id,
            ...
            .def("__repr__", [](const user_tally::bin_var_t &b) {
                ...
                if (!b.V.empty())
                    ax.push_back("V");
                if (!b.atom_id.empty())
                    ax.push_back("atom_id");
                if (!b.recoil_id.empty())
                    ax.push_back("recoil_id");
    ...
    utally_cls.def(py::init<>())
            .def_readwrite("id", &user_tally::parameters::id, "Unique tally identifier.")
            ...
            .def("__repr__",
                 [](const user_tally::parameters &p) { return "UserTally(id=\"" + p.id + "\")"; });
    bind_dict_access(utally_cls);
    py::bind_vector<std::vector<user_tally::parameters>>(m, "UserTallyList");

    py::class_<mccore::parameters> sim_cls(
            m, "SimulationParams", "Physics model selection.  Access via config.Simulation.");
    sim_cls.def(py::init<>())
            .def_readwrite("simulation_type", &mccore::parameters::simulation_type,
                           "Simulation type (SimulationType).")
            .def_readwrite("screening_type", &mccore::parameters::screening_type,
                           "Nuclear scattering potential (ScreeningType).")
            .def_readwrite("electronic_stopping", &mccore::parameters::electronic_stopping,
                           "Electronic stopping model (Stopping).")
            .def_readwrite("electronic_straggling", &mccore::parameters::electronic_straggling,
    ...
    py::class_<mccore::transport_options> tr_cls(
            m, "TransportParams", "Ion transport algorithm options.  Access via config.Transport.");
    tr_cls.def(py::init<>())
            .def_readwrite("flight_path_type", &mccore::transport_options::flight_path_type,
                           "Flight path algorithm (FlightPath).")
            .def_readwrite("flight_path_const", &mccore::transport_options::flight_path_const,
                           "Constant MFP value [atomic radii].  Used when FlightPath.Constant.")
            .def_readwrite("min_energy", &mccore::transport_options::min_energy,
                           "Stop tracking ion below this energy [eV].")
            .def_readwrite("min_recoil_energy", &mccore::transport_options::min_recoil_energy,
                           "Stop tracking recoil below this energy [eV].")
            .def_readwrite("min_scattering_angle", &mccore::transport_options::min_scattering_angle,
                           "Skip collisions below this angle [degrees].")
            .def_readwrite("max_rel_eloss", &mccore::transport_options::max_rel_eloss,
    ...
    py::class_<ion_beam::energy_distribution_t> edist_cls(
            m, "EnergyDistribution",
            "Ion beam energy distribution.  Access via config.IonBeam.energy_distribution.");
    edist_cls.def(py::init<>())
            .def_readwrite("type", &ion_beam::energy_distribution_t::type,
                           "Distribution type (Distribution).")
            .def_readwrite("center", &ion_beam::energy_distribution_t::center, "Mean energy [eV].")
    ...
    py::class_<ion_beam::spatial_distribution_t> sdist_cls(
            m, "SpatialDistribution",
            "Ion beam spatial distribution.  Access via config.IonBeam.spatial_distribution.");
    sdist_cls.def(py::init<>())
            ...
            .def_property(
                    "center", [](const ion_beam::spatial_distribution_t &s) { return s.center; },
    ...
    py::class_<ion_beam::parameters> ib_cls(
            m, "IonBeamParams", "Ion beam source definition.  Access via config.IonBeam.");
    ib_cls.def(py::init<>())
            .def_readwrite("ion", &ion_beam::parameters::ion, "Beam ion species (Element).")
    ...
    py::class_<target::target_desc_t> tgt_cls(
            m, "TargetParams",
    ...
    run_cls.def(py::init<>())
            .def_property(
                    ...
                    [](mcconfig::run_options &r, long long v) {
                        if (v < 0)
                            throw std::invalid_argument("max_no_ions must be >= 0");
            ...
            .def_readwrite("threads", &mcconfig::run_options::threads,
                           "Worker threads.  <= 0 = auto (half of hardware_concurrency if >3 "
                           "cores, else 1).")
            .def_property(
                    "seed", [](const mcconfig::run_options &r) { return (unsigned int)r.seed; },
    ...
    out_cls.def(py::init<>())
            .def_readwrite("title", &mcconfig::output_options::title, "Simulation title string.")
...
Methods::
    ...
    cfg_cls.def(py::init<>(), "Create a Config with all options at their default values.");
    ...
    cfg_cls.def(py::init([](const std::string &filename) {
                    ...
                    if (cfg.parseJSON(f, /*doValidation=*/true, &err) != 0)
                        throw py::value_error("Config: parse error in \"" + filename
                                              + "\": " + err.str());
                    return cfg;
                }),
                py::arg("filename"),
    ...
    cfg_cls.def_readwrite("Simulation", &mcconfig::Simulation)
    ...
    cfg_cls.def(
            ...
            .def(
                    "to_json", [](const mcconfig &cfg) { return cfg.toJSON(); },
            ...
            .def_static(
                    ...
                    py::arg("json_string"),
                    "Construct a Config from a JSON string.  Does not validate; call "
                    "config.validate() before use.")

            .def_static(
                    "options_spec", []() { return mcconfig::options_spec(); },
