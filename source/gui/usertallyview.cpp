...
public:
    ...
    {
        // Only show variables not already used by other rows
        const auto *m = static_cast<const BinVariableModel *>(index.model());
        QSet<QString> used;
        for (int r = 0; r < m->rowCount(); ++r) {
            if (r == index.row()) continue;
            used.insert(m->data(m->index(r, 0)).toString());
        }
        ...
        // CRITICAL: commit to model immediately when user picks a new variable
        connect(cb, QOverload<int>::of(&QComboBox::activated),
                [cb, this]() {
                    emit const_cast<VariableComboDelegate *>(this)->commitData(cb);
                });
...
bool BinVariableModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
    ...
    if (index.column() == 0)
        ...
        // Normalize: re-serialize parsed edges so garbage tokens (e.g. "8}") are visibly dropped
        const std::vector<float> parsed = parseEdges(value.toString());
        r.edges = parsed.size() >= 2 ? edgesToString(parsed) : value.toString();
    }
    else
        return false;
...
bool TallyTemplateDialog::loadTemplates()
{
    ...
    for (const auto &node : j) {
        ...
        e.parameters.event = parseEvent(
                QString::fromStdString(node.value("event", std::string("IonStop"))));

        if (node.contains("bins") && node["bins"].is_object()) {
            for (auto it = node["bins"].begin(); it != node["bins"].end(); ++it) {
                if (!it.value().is_array())
                    continue;
                std::vector<float> edges = it.value().get<std::vector<float>>();
                const std::string k = it.key();
                if (k == "x")         e.parameters.bins.x = edges;
                else if (k == "y")    e.parameters.bins.y = edges;
                else if (k == "z")    e.parameters.bins.z = edges;
                else if (k == "r")    e.parameters.bins.r = edges;
                else if (k == "rho")  e.parameters.bins.rho = edges;
                else if (k == "cosTheta") e.parameters.bins.cosTheta = edges;
                else if (k == "nx")   e.parameters.bins.nx = edges;
                else if (k == "ny")   e.parameters.bins.ny = edges;
                else if (k == "nz")   e.parameters.bins.nz = edges;
                else if (k == "E")    e.parameters.bins.E = edges;
                else if (k == "Tdam") e.parameters.bins.Tdam = edges;
                else if (k == "V")    e.parameters.bins.V = edges;
                else if (k == "atom_id")   e.parameters.bins.atom_id = edges;
                else if (k == "recoil_id") e.parameters.bins.recoil_id = edges;
            }
        }
      ...
      binModel_(new BinVariableModel(this))
{
    ...
    btDbTally_->setIcon(QIcon(":/assets/ionicons/document-text-outline.svg"));
    ...
    // Keep variable combo always visible — no click needed to see the dropdown
    connect(binModel_, &BinVariableModel::rowsChanged, this, [this]() {
        for (int r = 0; r < binModel_->rowCount(); ++r)
            binTableView_->openPersistentEditor(binModel_->index(r, 0));
    });

    // When a variable name changes in one row, close+reopen all persistent combos
    // so their item lists (which filter out already-used variables) stay current.
    // QueuedConnection: defer until after the activated signal stack unwinds,
    // so we never destroy the combo box while its signal handler is still running.
    connect(binModel_, &BinVariableModel::dataChanged, this,
            [this](const QModelIndex &tl, const QModelIndex &) {
                if (tl.column() != 0)
                    return;
                for (int r = 0; r < binModel_->rowCount(); ++r) {
                    binTableView_->closePersistentEditor(binModel_->index(r, 0));
                    binTableView_->openPersistentEditor(binModel_->index(r, 0));
                }
            }, Qt::QueuedConnection);
...
{
    // Pick the first variable not already used in any row
    const QStringList allVars = BinVariableModel::variableNames();
    const std::vector<BinVariableModel::Row> existing = binModel_->rows();
    QString pick;
    for (const QString &v : allVars) {
        bool used = false;
        for (const auto &r : existing)
            if (r.variable == v) { used = true; break; }
        if (!used) { pick = v; break; }
    }
    if (pick.isEmpty())
        return; // all 14 variables are already in use

    const int newRow = binModel_->rowCount();
    binModel_->insertRows(newRow, 1);
    binModel_->setData(binModel_->index(newRow, 0), pick);
}
...
void UserTallyView::updateSummaryLabel()
{
    ...
    // Disable [+] when all 14 variables are already in use
    if (currentTally()) {
        const bool hasRoom = binModel_->rowCount()
                             < static_cast<int>(BinVariableModel::variableNames().size());
        btAddBin_->setEnabled(hasRoom);
    }
}
...
void UserTallyView::onUseLabFrameChanged(int)
{
    ...
    coordWidget_->setEnabled(!useLab);
    setValueData();
}
